...
fun SeriesDetailScreen(
    ...
    Column(modifier = modifier.fillMaxSize()) {
        DetailTopBar(title = (state as? UiState.Success)?.data?.title ?: "Series", onBack = onBack)

        when (val s = state) {
            is UiState.NotConfigured -> NotConfiguredPlaceholder("SONARR", onBack, Modifier.fillMaxSize())
    ...
    Column(modifier = modifier.verticalScroll(rememberScrollState())) {
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.padding(bottom = 16.dp)) {
                StatusBadge(text = "${series.episodeFileCount}/${series.episodeCount} episodes", color = StatusSuccess)
                StatusBadge(
                    text = if (series.monitored) "Monitored" else "Unmonitored",
                    color = if (series.monitored) AccentGreen else TextMuted,
                )
            }
